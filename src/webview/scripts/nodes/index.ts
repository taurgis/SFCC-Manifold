/**
 * Node drawing module
 * Handles rendering nodes on the canvas
 */

export function getNodeScript(): string {
  return `
    /**
     ...
     * Add navigation indicator for jump/call nodes
     */
    function addNavigationIndicator(group, nodeWidth, nodeHeight) {
     ...
     */
    function setupNodeHover(group, rect, node, layer) {
      group.on("mouseenter", function() {
        document.body.style.cursor = "pointer";
        if (selectedNodeId !== node.id) {
